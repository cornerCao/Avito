...
train_features = pd.read_csv('../input/aggregated-features-lightgbm/aggregated_features.csv')
...
#EMBEDDING_FILE1 = '../input/russian-glove/multilingual_embeddings.ru'  #glove
...
def RNN_model():
    ...
    # model
    model = Model([seq_title_description, region, city, category_name, parent_category_name, param_1, param123, price,
                   item_seq_number, image_code, avg_ad_days, avg_ad_times, n_user_items], output)
...
for train_idx, test_idx in skf.split(train1, y_train):
    ...
    # Fit the NN Model
    for i in range(3):
        hist = modelRNN.fit(X_train_f, y_train1, batch_size=batch_size + (batch_size * (2 * i)), epochs=epochs,
...
print(pred_final1[0:5])
print(pred_final2[0:5])
